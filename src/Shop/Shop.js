...
export default class Shop extends Component {
    ...
    render() {
        const scrolltosells = () => {
            if (this.state.all === "true" && window.innerWidth > 1642) {
                window.scrollTo({ top: 2620, left: 0, behavior: 'smooth' });
            }
         else if (this.state.all === "true" && window.innerWidth < 1642) {
                window.scrollTo({ top: 2320, left: 0, behavior: 'smooth' });
            }

            else {
                window.scrollTo({ top: 1180, left: 0, behavior: 'smooth' });
            }
        };
